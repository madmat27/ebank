<?php

            session_start(); // Ανοίγουμε session για να μένει συνδεδεμένος ο authenticated χρήστης
            include './scripts/functionlib.php'; // Eισάγουμε την σελίδα που έχουμε τη βιβλιοθήκη με τις μεθόδους
            require './scripts/DBconnect.php'; // Eισάγουμε την σελίδα που έχει db connection
            userValidate(); //Πιστοποίηση χρήστη με το session που έχουμε ανοιχτό
            
            // Εύρεση στοιχείων πελάτη κι εκχώρηση μεταβλητών
            $cusID = $_SESSION['user_id'];
            $name = customerName();
            $lastname = customerLNAME();
            $email = customerEmail();
            
            $query = "SELECT SUM(`balance`) FROM `Accounts` WHERE `cusID` = '".$cusID."'";
            $result = mysqli_query($con,$query);
            $row = mysqli_fetch_row($result);
            $accountSum = number_format($row[0],2,",",".") ;
            
?>

<!DOCTYPE html>
<html lang="en">
    <head>

        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <meta name="description" content="iBank: Η τράπεζά σας στις υπηρεσίες σας">
        <meta name="author" content="Mariana S. Mazi">
        <meta name="keywords" content="iBank, ebanking, ιδιώτες">

        <title>iBank | Στατιστικά</title>

        <!-- Custom fonts for this template -->
        <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.6.3/css/all.css" integrity="sha384-UHRtZLI+pbxtHCWp1t77Bi1L4ZtiqrqD80Kn4Z8NTSRyMA2Fd33n5dQ8lWUE00s/" crossorigin="anonymous">
        <link href="https://fonts.googleapis.com/css?family=Montserrat:400,700" rel="stylesheet" type="text/css">
        <link href="https://fonts.googleapis.com/css?family=Kaushan+Script" rel="stylesheet" type="text/css">
        <link href="https://fonts.googleapis.com/css?family=Droid+Serif:400,700,400italic,700italic" rel="stylesheet" type="text/css">
        <link href="https://fonts.googleapis.com/css?family=Roboto+Slab:400,100,300,700" rel="stylesheet" type="text/css">
        <link href="https://fonts.googleapis.com/css?family=Didact+Gothic" rel="stylesheet" type="text/css"> 
        
        <!-- Favicon! :-) -->
        <link rel="icon" type="image/png" href="./icons/favicon.png">
        
        <!-- CSS Stylesheet -->
        <link rel = "stylesheet" type = "text/css" href = "./styles/mainstyle.css" />

    </head>

    <body>
        <!-- Χώρισα τη σελίδα σε 3 κεντρικά κομμάτια και δημιούργησα τα αντίστοιχα divs.
             Έχουμε λοιπόν την κεφαλίδα, που ορίζεται από το div με id="header", το υποσέλιδο που ορίζεται από 
             το div με id="footer" & το κυρίως κομμάτι που ορίζεται από το div container. 
             Εδώ να σημειώσουμε ότι τα header & footer θα είναι κοινά σε όλες τις σελίδες (με κάποιες ίσως μικρές αλλαγές), 
             ενώ το περιεχόμενο του container αλλάζει ανάλογα με τη σελίδα (εξού και το όνομα).
             Παρακάτω ξεκινά το header: -->
        <div id="header">
            
            <div>
                <a href="./ebanking.php"><img id="logo" src="./images/logo.png"></a>
                
                <!-- Στο παρακάτω div δημιουργούμε έναν πίνακα που να περιλαμβάνει τις 4 επιλογές που υπάρχουν στη σελίδα μας:
                     Τη "Βοήθεια", την "Αναζήτηση", την "Χρήστης" και την "Αποσύνδεση".  -->
                <div id="header-container">
                    
                    <table>
                        
                        <tr>
                            
                            <td style="width: 100px;">
                                
                                <div id="help">
                                    <span style="font-size: 22px">
                                    <i class="far fa-question-circle"></i>
                                </span>

                                <a href="#">Βοήθεια</a>

                                </div>
                                
                            </td>
                            
                            <td>
                                
                                <div id="search">

                                    <span style="font-size: 22px; ">
                                        <i class="fas fa-search"></i>
                                    </span>

                                    <input id="searchbox" type="text" placeholder="Αναζήτηση">

                                </div>
                                
                            </td>
                            
                        </tr>
                        
                        <tr>
                            
                            <td style="width: 200px;">
                                
                                <div id="user">
                                    <span style="font-size: 22px">
                                        <i class="fas fa-user"></i>
                                    </span>
                                
                                    
                                    &nbsp;<a href="./user.php">Γεια σου, <?php echo $name; ?></a>

                                </div>
                                
                            </td>
                            
                            <td>
                                
                                <div id="logout">

                                    <span style="font-size: 22px; ">
                                        <i class="fas fa-sign-out-alt"></i>
                                    </span>

                                    <a href="./scripts/logout.php">Αποσύνδεση</a>

                                </div>
                                
                            </td>
                            
                        </tr>
                        
                    </table>
                    
                </div>

            </div>
            
            <!-- Το παρακάτω div αποτελεί το μενού που είναι εμφανές σε όλες τις σελίδες. Η μορφοποίηση γίνεται με τους css selectors: 
                 τα ids & τις κλάσεις. Παρατηρούμε ότι στην παρούσα σελίδα, το μενού αλλάζει για να προσαρμοστεί με την σελίδα του 
                 e-banking. -->
            <div id="navigation">
                <div class="menu">
                    
                    <a href="./ebanking.php">Αρχική</a>
                
                </div>
                
                <div class="menu">
                    
                    <a href="./accounts.php">Λογαριασμοί</a>
                
                </div>

                <div class="menu">
                    
                    <a href="#">Κάρτες</a>
                
                </div>

                <div class="menu">
                    
                    <a href="./transfer.php">Μεταφορές</a>
                
                </div>
                
                <div class="menu">
                    
                    <a href="./payments.php">Πληρωμές</a>
                
                </div>
                
                <div class="menu">
                    
                    <a href="./history.php">Ιστορικό</a> 
                
                </div>                

                <div class="menu">
                    
                    <a href="./user.php">Προφίλ Χρήστη</a>
                
                </div>

                <div class="menu menu-selected">
                    
                    <a href="./statistics.php">Στατιστικά</a>
                
                </div>   
                
            </div>
        
        </div>
        
        <!-- Εδώ ξεκινά το container. Για κάθε σελίδα της άσκησης είναι διαφορετικό & μας επιτρέπει να αλλάζουμε περιεχόμενο
             χωρίς να αλλοιώνουμε το branding της ιστοσελίδας. Παρακάτω είναι το περιεχόμενο της σελίδας transfer (Μεταφοράς 
             χρημάτων - Εικόνα 3 της εκφώνησης). -->
        <div id="container">
            
            <div id="style-container">
                
                <!-- Αυτό χρησιμοποιείται καθαρά για σχεδιαστικούς λόγους, για να αφήσει το απαιτούμενο κενό. -->
                <div class="smallspace">

                </div>
                
                <!-- Εδώ ξεκινούν τα πλαίσια που περιλαμβάνουν τους λογαριασμούς & τις κάρτες. Γίνεται κάποια επιπλέον μορφοποίηση
                     με την ιδιότητα 'style', γιατί είναι μεμονωμένες περιπτώσεις κι εξυπηρετεί καλύτερα η άμεση εφαρμογή css κανόνων-->
                <div id="stats-mainbox"> 

                    <p class="regtitle regtitle-extra">Στατιστικά χρήσης</p>
                    
                    <div class="overview"> 
                        
                        <table class="overview-table">
                            <tr>
                                <td colspan = "2" style = "background-color: #eeeeee;padding-left:20px"><span class="overview-title">Με μια ματιά: </span></td>
                            </tr>
                            <tr>
                                <td style = "width:55%;padding-left:20px"><span class="overview-caption">Συνολικό Διαθέσιμο Υπόλοιπο Λογαριασμών:</span>&nbsp</td>
                                <td style = "width:50%;"><span class="overview-sum">&nbsp<?php echo $accountSum; ?></span></td>
                            </tr>
                        </table>
                        
                    </div>
                    
                    <div class="space"> </div>
                    
                    <div class="space"> <hr> </div>
                    
                    <p>Θέλω να δω:</p>
                    
                    <div id="accounts" class="transfer-form">
                        
                        
                    <button class="section">
                        
                        Το διαθέσιμο υπόλοιπο ανά λογαριασμό
                    
                    </button>
                    
                    <div class="panel">
                    
                        <div id="accounts" class="infogrid">
                            
                            <div class="smallspace"></div>
                        
                            <?php
    
                                // Εύρεση στοιχείων λογαριασμού του συγκεκριμένου πελάτη 
                                $query = "SELECT * FROM `Accounts` WHERE `cusID` = '".$cusID."'";
                                $result = mysqli_query($con,$query);
                                $i = 0;
                                
                                echo "<table class=\"payments-table\">";
                                echo "<tr class=\"payments-table-header\">";
                                echo "<th>Φιλικό Όνομα</th>";
                                echo "<th>IBAN</th>";
                                echo "<th>Διαθέσιμο Υπόλοιπο</th>";
                                echo "</tr>";
                                
                                while ($row = mysqli_fetch_row($result)) {
                                    
                                    $friendlyName = $row[2];
                                    $myIBAN = $row[3];
                                    $amount = $row[5];
                                    
                                    
                                    echo ($i % 2)?'<tr class="payments-table-oddlines">':'<tr class="payments-table-evenlines">';
                                    echo "<td>";
                                    echo "<span>".$friendlyName."</span>";
                                    echo "</td>";
                                    echo "<td>";
                                    echo "<span>".$myIBAN."</span>";
                                    echo "</td>";
                                    echo "<td>";
                                    
                                    if ($amount < 0) {
                                        
                                        echo "<div class=\"card-amount-debit\">".number_format($amount,2,",",".")." Ευρώ</div>";
                                    
                                    } else {
                                        echo "<div class=\"card-amount-credit\">".number_format($amount,2,",",".")." Ευρώ</div>";
                                        
                                    }
                                    
                                    echo "</td>";
                                    echo "</tr>";
                                    $i++;
                                    
                                }
    
                                echo "</table>";
    
                            ?>
                            
                            <div class="smallspace"></div>

                        </div>
                    
                    <!-- Αυτό χρησιμοποιείται καθαρά για σχεδιαστικούς λόγους, για να αφήσει το απαιτούμενο κενό. -->
                    <div class="space">
    
                    </div>
                    
                    </div>
                    
                    <button class="section">
                        
                        Το ιστορικό συναλλαγών ανά λογαριασμό
                        
                    </button>
                    
                    <div class="panel">
                    
                        <form name="account-history" action="statistics.php" method="post">
                            
                            <div class="smallspace"></div>
                            
                            <table style="width: 100%;">
                                
                                <tr style="height: 35px;">
                                    
                                   <td style="text-align: right; width: 30%">

                                        <p>Παρακαλώ επιλέξτε λογαριασμό: </p>

                                    </td>

                                    <td style="text-align: left">
                                        
                                        <!-- Στοιχείο επιλογών: περιλαμβάνει όλους τους λογαριασμούς που έχουμε διαθέσιμους στην σελίδα ebanking: -->
                                        <select name="accountname" required class="transfer-inputs" style="width: 100%">
                                            
                                            <?php
            
                                                // Εύρεση στοιχείων λογαριασμού του συγκεκριμένου πελάτη 
                                                $query  = "SELECT *  FROM `Accounts` WHERE cusID = '".$_SESSION['user_id']."'";
                                                $result = mysqli_query($con,$query);
                                                
                                                while ($row = mysqli_fetch_row($result)) {

                                                    $accountID = $row[0];
                                                    $friendlyName = $row[2];
                                                    $iban = $row[3];
                                                    $doc = $row[4];
                                                    $balance = $row[5];

                                                    if ($balance < 0) {
                                                        
                                                        echo "<div class=\"card-amount-debit\">".number_format($balance,2,",",".")." Ευρώ</div>";
                                                    
                                                    } else {
                                                        echo "<div class=\"card-amount-credit\">".number_format($balance,2,",",".")." Ευρώ</div>";
                                                        
                                                    }
                                                    
                                                    echo "<option value=".$accountID.">".$friendlyName." (".number_format($balance,2,",",".").") - ".$iban."</option>";

                                                }

                                            ?> 

                                        </select>

                                    </td>
                                    
                                    <td>
                                        
                                        <!-- Κουμπί υποβολής φόρμας --> 
                                        <p>&nbsp<input style="width: 200px; height: 25px;" name="accn_submit" id="accn_submit" type="submit" value="Επιλογή"></p>
                                        
                                    </td>

                                </tr>
                                
                            </table>
                                
                        <div class="smallspace"> </div>
                
                    <hr>
                        
                    <div id="accounts" class="infogrid">
                        
                        <?php

                            // Εύρεση στοιχείων λογαριασμού του συγκεκριμένου πελάτη 
                            
                            if (isset($_POST['accn_submit'])) {
                                
                                
                                $accountID = $_POST['accountname'];
                                $query ="SELECT M.dot AS \"DATE\",\n"
                                    . " A.cusID AS \"CUSTOMERID\",\n"
                                    . " A.friendlyName AS \"FROM (Name)\",\n" 
                                    . " (SELECT CONCAT(C.name,' ',C.lastname)\n"
                                    . " FROM Customers C INNER JOIN Accounts A ON C.customerID = A.cusID WHERE A.accountID = M.creditAccountID) AS \"PAY TO\",\n" 
                                    . " B.IBAN AS \"To (IBAN)\",\n" 
                                    . " M.amount AS \"VALUE\",\n" 
                                    . " M.INfos AS \"INFO\",\n" 
                                    . " \"Μ\" AS \"TYPE OF PAYMENT\"\n"
                                    . " FROM MoneyTransfer M\n"
                                    . " INNER JOIN Accounts A ON M.billingAccountID = A.accountID\n"
                                    . " INNER JOIN Accounts B ON M.creditAccountID = B.accountID\n"
                                    . " WHERE billingAccountID = '".$accountID."'\n"
                                    . " OR creditAccountID = '".$accountID."'\n"
                                    . " UNION ALL\n"
                                    . " SELECT P.dot AS \"DATE\",\n" 
                                    . " P.customerID AS \"CUSTOMERID\",\n"
                                    . " A.friendlyName AS \"FROM (Name)\",\n" 
                                    . " PT.paymentType AS \"PAY TO\",\n" 
                                    . " NULL AS \"To (IBAN)\",\n" 
                                    . " P.amount AS \"VALUE\",\n" 
                                    . " P.paymentCode AS \"INFO\",\n" 
                                    . " \"Π\" AS \"TYPE OF PAYMENT\"\n"
                                    . " FROM Payments P\n"
                                    . " INNER JOIN Accounts A ON P.accountID = A.accountID\n"
                                    . " INNER JOIN PaymentType PT ON P.paymentTypeID = PT.paymentTypeID\n"
                                    . " WHERE P.accountID= '".$accountID."'\n"
                                    . " ORDER BY DATE DESC\n";
                                $result = mysqli_query($con,$query);
                                $i = 0;
                            
                                echo "<table class=\"payments-table\">";
                                echo "<tr class=\"payments-table-header\">";
                                echo "<th>Ημ/νία Συναλλαγής</th>";
                                echo "<th>Από λογαριασμό</th>";
                                echo "<th>Προς δικαιούχο</th>";
                                echo "<th>IBAN</th>";
                                echo "<th>Ποσό</th>";
                                echo "<th>Σχόλια</th>";
                                echo "<th>Συναλλαγή</th>";
                                echo "</tr>";
                                
                                while ($row = mysqli_fetch_row($result)) {
                                
                                    $transactionDate = $row[0];
                                    $friendlyName = $row[2];
                                    $beneficiary = $row[3];
                                    $befIBAN = $row[4];
                                    $amount = $row[5];
                                    $rems = $row[6];
                                    $transactionType = $row[7];

                                    
                                    echo ($i % 2)?'<tr class="payments-table-oddlines">':'<tr class="payments-table-evenlines">';
                                    echo "<td>";
                                    echo "<span>".date("d/m/Y h:m:s", strtotime($transactionDate))."</span>";
                                    echo "</td>";
                                    echo "<td>";
                                    echo "<span>".$friendlyName."</span>";
                                    echo "</td>";
                                    echo "<td>";
                                    echo "<span>".$beneficiary."</span>";
                                    echo "</td>";
                                    echo "<td>";
                                    
                                    if ($befIBAN) {
                                        
                                        echo "<span>".$befIBAN."</span>";
                                    
                                    } else {
                                        
                                        echo "<span>---</span>";
                                        
                                    }
                                    
                                    echo "</td>";
                                    echo "<td>";
                                    echo "<span>".number_format($amount,2,",",".")."</span>";
                                    echo "</td>";
                                    echo "<td>";
                                    echo "<span>".$rems."</span>";
                                    echo "</td>";
                                    echo "<td>";
                                    
                                    if ($transactionType == 'Μ') {
                                        
                                        echo "<span>Μεταφορά</span>";
                                        
                                    } else {
                                        
                                        echo "<span>Πληρωμή</span>";
                                    }
                                    
                                    echo "</td>";
                                    echo "</tr>";
                                    $i++;
                                    
                                }
                                
                            }
                            
                           echo "</table>";

                        ?>

                        </div>
                        </form>
                    
                        <!-- Αυτό χρησιμοποιείται καθαρά για σχεδιαστικούς λόγους, για να αφήσει το απαιτούμενο κενό. -->
                        <div class="space"></div>
                        
                        <div class="space"></div>
                    
                    </div>
                    
                    <button class="section">
                    
                        To συνολικό ποσό πληρωμών ανά κατηγορία
                    
                    </button>
                    
                    <div class="panel">
                        
                        <form name="payment-stats" action="statistics.php" method="post">
                        
                        <div class="smallspace"></div>
                            
                            <table style="width: 100%;">
                                
                                <tr style="height: 35px;">
                                    
                                   <td style="text-align: right; width: 30%">

                                        <p>Παρακαλώ επιλέξτε ημερομηνία από: </p>

                                    </td>
                                    
                                    <td style="text-align: left; width: 10%">

                                        <p><input type="date" name="from_date" required></p>

                                    </td>
                                    
                                    <td style="text-align: right; width: 7%">

                                        <p>Έως: </p>

                                    </td>
                                    
                                    <td style="text-align: left; width: 20%">

                                        <p><input type="date" name="to_date" required></p>

                                    </td>
                                    
                                    <td style="text-align: left; width: 33%">

                                        <!-- Κουμπί υποβολής φόρμας --> 
                                        <p>&nbsp<input style="width: 200px; height: 25px;" name="stats_submit" id="accn_submit" type="submit" value="Εύρεση"></p>
                                        

                                    </td>
                                    
                                </tr>
                                
                            </table>
                                
                            <div class="smallspace"> </div>
                
                            <hr>
                                
                                <div class="smallspace"> </div>
                                
                                <div id="accounts" class="infogrid">
                    
                                    <?php
                                    
                                        if (isset($_POST['stats_submit'])) {
                                            
                                            $fromDate = date("Y-m-d 00:00:00", strtotime($_POST['from_date']));
                                            $toDate = date("Y-m-d 00:00:00", strtotime($_POST['to_date']));

                                            // Εύρεση συνόλου πληρωμών σε εφορία
                                            $query = "SELECT SUM(amount) FROM `Payments` WHERE dot > '".$fromDate."' AND dot < '".$toDate."' AND `paymentTypeID` = 1 AND customerID = '".$cusID."'";
                                            $result = mysqli_query($con,$query);
                                        
                                        
                                            while ($row = mysqli_fetch_row($result)) {
                                                
                                                $irsSum = $row[0];
                                                
                                            }
                                        
                                        
                                            $query = "SELECT SUM(amount) FROM `Payments` WHERE dot > '".$fromDate."' AND dot < '".$toDate."' AND `paymentTypeID` = 2 AND customerID = '".$cusID."'";
                                            $result = mysqli_query($con,$query);
                                            
                                            while ($row = mysqli_fetch_row($result)) {
                                                
                                                $enfiaSum = $row[0];
                                                
                                            }

                                            $query = "SELECT SUM(amount) FROM `Payments` WHERE dot > '".$fromDate."' AND dot < '".$toDate."' AND `paymentTypeID` = 3 AND customerID = '".$cusID."'";
                                            $result = mysqli_query($con,$query);
                                            
                                            while ($row = mysqli_fetch_row($result)) {
                                                
                                                $deiSum = $row[0];
                                                
                                            }
                                            
                                            $query = "SELECT SUM(amount) FROM `Payments` WHERE dot > '".$fromDate."' AND dot < '".$toDate."' AND `paymentTypeID` = 4 AND customerID = '".$cusID."'";
                                            $result = mysqli_query($con,$query);
                                            
                                            while ($row = mysqli_fetch_row($result)) {
                                                
                                                $carInsuSum = $row[0];
                                                
                                            }
                                            
                                            echo "<table class=\"payments-table\" style=\"width:40%; margin-left: auto; margin-right: auto\">";
                                            echo "<tr>";
                                            echo "<td>";
                                            echo "<th class=\"payments-table-header\" style=\"width:50%\">Εφορία</th>";
                                            echo "</td>";
                                            echo "<td class=\"payments-table-oddlines\">";
                                            echo "<span>".$irsSum."</span>";
                                            echo "</td>";
                                            echo "</tr>";
                                            echo "<tr>";
                                            echo "<td>";
                                            echo "<th class=\"payments-table-header\" style=\"width:50%\">ΕΝΦΙΑ</th>";
                                            echo "</td>";
                                            echo "<td class=\"payments-table-evenlines\">";
                                            echo "<span>".$enfiaSum."</span>";
                                            echo "</td>";
                                            echo "</tr>";
                                            echo "<tr>";
                                            echo "<td>";
                                            echo "<th class=\"payments-table-header\" style=\"width:50%\">ΔΕΗ</th>";
                                            echo "</td>";
                                            echo "<td class=\"payments-table-oddlines\">";
                                            echo "<span>".$deiSum."</span>";
                                            echo "</td>";
                                            echo "</tr>";
                                            echo "<tr>";
                                            echo "<td>";
                                            echo "<th class=\"payments-table-header\" style=\"width:50%\">Ασφάλεια Αυτοκινήτου</th>";
                                            echo "</td>";
                                            echo "<td class=\"payments-table-evenlines\">";
                                            echo "<span>".$carInsuSum."</span>";
                                            echo "</td>";
                                            echo "</tr>";
                                            echo "</table>";
                                            
                                            echo "<div class=\"space\"></div>";
                                            
                                            $highestPayment = "";
                                            
                                            if ($irsSum > $enfiaSum) {
                                                
                                                $highestPayment = "Εφορία";
                                                
                                            } else {
                                                
                                                $highestPayment = "ΕΝΦΙΑ";
                                                
                                            }
                                            
                                            if ($deiSum > $carInsuSum) {
                                                
                                                if ($deiSum > $highestPayment) {
                                                    
                                                    $highestPayment = "ΔΕΗ";
                                                    
                                                }
                                                
                                            } else {
                                                
                                                if ($carInsuSum > $highestPayment) {
                                                    
                                                    $highestPayment = "Ασφάλεια Αυτοκινήτου";
                                                }
                                            }
                                            
                                            echo "<p class=\"overview-caption\" style=\"text-align:center\">Η κατηγορία με το μεγαλύτερο ποσό πληρωμής είναι η <span class=\"overview-sum\">".$highestPayment."</span></p>";

                                        }
            
                                    ?>
            
                                </div>
                            
                            <div class="space"> </div>        
                    
                    </div>

                </div>
             
            </div>            
            
        </div>
        
        <!-- Αυτό χρησιμοποιείται καθαρά για σχεδιαστικούς λόγους, για να αφήσει το απαιτούμενο κενό. -->
        <div class="space"></div>
        <div class="space"></div>

        <!-- Εδώ ξεκινάει το υποσέλιδο. Χρησιμοποιώ 3 divs για να περιλάβω το email, τα τηλέφωνα και τα links για τους 
             όρους χρήσης και την πολιτική απορρήτου. Εδώ αξίζει να προσέξουμε ότι τα divs δεν είναι όμοια, αλλά κάθε ένα έχει το δικό
             του πλάτος. Εδώ εξυπηρετεί να χρησιμοποιήσουμε το style αντί για css, γιατί οι διαφορές είναι πιο πολλές απ' όσες οι ομοιότητες, 
             ώστε να μας επιτρέπει να δημιουργήσουμε κλάσεις. -->
        <div id="footer">
            
            <div id="mail">
                
                <table> 
                    
                    <tr>
                        
                        <td>
                            
                            <span style="font-size: 48px; ">

                                <i class="far fa-envelope"></i>

                            </span>
                            
                        </td>
                        
                        <td class="seperator" style="width: 250px;">
                            
                            <a href="mailto:support@ibank.gr">support@ibank.gr</a><br>
                            <span style="font-size: 14px;">Email Επικοινωνίας</span>
                            
                        </td>
                        
                    </tr>
                    
                </table>
    
            </div>
                        
            <div id="phones">
                
                <table>
                    
                    <tr style="width: 450px;">
                        
                        <td style="padding-right: 10px;">
                            
                            <span style="font-size: 48px; ">
                            
                                <i class="fas fa-phone"></i>
                            
                            </span>
                        
                        </td>
                        
                        <td style="padding-right: 10px;">                           
                            
                            123456 <br>
                            <span style="font-size: 14px;">Κλήση από Ελλάδα</span>
                            
                        </td>
                        
                        <td>                           
                            
                            +30 2101234567 <br>
                            <span style="font-size: 14px;">Κλήση από Εξωτερικό</span>
                            
                        </td>
                        
                    </tr>
                </table>
            
            </div>
            
            <div id="terms-of-use">
                
                <a href="#">Όροι Χρήσης</a> | <a href="#"> Πολιτική απορρήτου </a>
                
            </div>
        
        </div>
        
        <!-- Javascript κώδικας -->
        <!-- Να σημειωθεί ότι όταν βάζουμε τον κώδικα μέσα στη σελίδα, προτιμούμε να τον βάζουμε στο τέλος του body, ώστε να μην καθυστερεί
             το φόρτωμα της σελίδας, για να γίνει compile το js. -->
        <script>
        
            function validateForm() {
                
                 
                
            }
            
            /* JS script για τη λειτουργία του Section 
            var acc = document.getElementsByClassName("section");
            var i;
            
            for (i = 0; i < acc.length; i++) {
            
                acc[i].addEventListener("click", function() {
                /* Εναλλαγή μεταξύ της προσθήκης και της αφαίρεσης της ενεργής κλάσης,
                   που δίνει έμφαση στο κουμπί που ελέγχει το panel 
                this.classList.toggle("active");
            
                /* Εναλλαγή μεταξύ της εμφάνισης και της απόκρυψης του ενεργού panel 
                var panel = this.nextElementSibling;
                if (panel.style.maxHeight) {
                
                    panel.style.maxHeight = null;
            
                    
                } else {
            
                    panel.style.maxHeight = panel.scrollHeight + "px";
                    
                }
            });
            
            }*/
            
         </script>
    
    
    </body>
</html>